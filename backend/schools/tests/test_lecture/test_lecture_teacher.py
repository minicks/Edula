...
class TestLectureTeacher(TestSetUp):
    """Test lecture Teacher
    teacher1 기준으로 테스트 진행
    """
    def setUp(self) -> None:
        res = self.client.post(
            self.jwt_url,
            self.teacher1_info_json,
            content_type=self.content_type
        )
        self.teacher1_token = res.data.get('access')
        return super().setUp()
    ...
    def test_lecture_list(self):
        ...
        self.client.credentials(
            HTTP_AUTHORIZATION=f'JWT {self.teacher1_token}'
        )
        res = self.client.get(url)
